    ...
    try:
        if message.command[1] == "shutdown":
            ...
            if "Already up to date." in str(out):
                ...
                await message.reply(f"<blockquote><code>{out}</code></blockquote>", quote=True)
            os.execl(sys.executable, sys.executable, "-m", "PyroUbot", tomi_k)
        elif message.command[1] == "clean":
            count = 0
            for file_name in os.popen("ls").read().split():
                if file_name not in ["start.sh", "requirements.txt"]:
                    try:
                        os.remove(file_name)
                        count += 1
...
async def cb_restart(client, callback_query):
    ...
    os.system(f"kill -9 {os.getpid()} && python3 -m PyroUbot {tomi_k}")


async def cb_gitpull(client, callback_query):
    await callback_query.message.delete()
    os.system(f"kill -9 {os.getpid()} && git pull && python3 -m PyroUbot {tomi_k}")

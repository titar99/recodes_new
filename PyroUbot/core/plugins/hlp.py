...
async def help_cmd(client, message):
    ...
    if not get_arg(message):
        ...
        if get_arg(message) in HELP_COMMANDS:
            await message.reply(
                HELP_COMMANDS[get_arg(message)].__HELP__.format(x) + f"\n<b>© {bot.me.mention} </b>",
                quote=True,
            )
        else:
...
async def menu_inline(client, inline_query):
    msg = f"<b>⌕ ʜᴇʟᴘ ᴍᴇɴᴜ </b>\n<b>ⲇ ᴜꜱᴇʀ :</b> <code><a href=tg://user?id={inline_query.from_user.id}>{inline_query.from_user.first_name} {inline_query.from_user.last_name or ''}</a></code>\n<b>ⲇ ᴛᴏᴛᴀʟ ᴍᴏᴅᴜʟᴇs: <code>{len(HELP_COMMANDS)}<?code>"
...
async def menu_callback(client, callback_query):
    ...
    home_match = re.match(r"help_home", callback_query.data)
    prefix = await ubot.get_prefix(client.me.id)
    top_text = f"<blockquote><b>⌕ ᴍᴇɴᴜ ɪɴʟɪɴᴇ<b>\n<b>ⲇ ᴜꜱᴇʀ :</b> <code><a href=tg://user?id={callback_query.from_user.id}>{callback_query.from_user.first_name} {callback_query.from_user.last_name or ''}</a></code>\n<b>ⲇ ᴛᴏᴛᴀʟ ᴍᴏᴅᴜʟᴇs: <code>{len(HELP_COMMANDS)}<?code></blockquote>"
    ...
    if home_match:
        button = [[InlineKeyboardButton("• ᴍᴇɴᴜ •", callback_data="help_back")]]
        await callback_query.edit_message_text(
            text="ᴋʟɪᴋ ᴍᴇɴᴜ ᴜɴᴛᴜᴋ ᴍᴇɴᴀᴍᴘɪʟᴋᴀɴ ғɪᴛᴜʀ",
            reply_markup=InlineKeyboardMarkup(button),
            disable_web_page_preview=True,
  )

...
async def restart_cmd(client, message):
    ...
    if message.from_user.id not in ubot._get_my_id:
        return await msg.edit(f"<blockquote><b>ᴀɴᴅᴀ ᴛɪᴅᴀᴋ ʙɪsᴀ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ᴘᴇʀɪɴᴛᴀʜ ɪɴɪ. ᴅɪᴋᴀʀᴇɴᴀᴋᴀɴ ᴀɴᴅᴀ ʙᴜᴋᴀɴ ᴘᴇɴɢɢᴜɴᴀ @{bot.me.username}</b></blockquote>")
    for X in ubot._ubot:
            ...
            for _ubot_ in await get_userbots():
                if X.me.id == int(_ubot_["name"]):
                    try:
                        ...
                        for mod in loadModule():
                            importlib.reload(importlib.import_module(f"PyroUbot.modules.{mod}"))
                        return await msg.edit(f"<blockquote><b><emoji id=6278555627639801385>✅</emoji> ʀᴇsᴛᴀʀᴛ ʙᴇʀʜᴀsɪʟ ᴅɪʟᴀᴋᴜᴋᴀɴ {UB.me.first_name} {UB.me.last_name or ''} | {UB.me.id}</b></blockquote>")
                    except Exception as error:
